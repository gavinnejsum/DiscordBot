...
function createSingleEventEmbed(event) {
  const eventName = editFunctions.capitalizeFirstLetter(event[0]);

  const eventData = getEventData.findEventData(eventName);
...
function officerTypeReaction(eventName) {
  const embed = new Discord.MessageEmbed()
  const eventNameLowerCase = eventName.toString().toLowerCase();

  const eventNameCopy = eventName[0].toUpperCase() + eventName.slice(1);
  embed.setTitle(eventNameCopy)
  embed.setDescription('React to chose what Crew type you are looking for :)')

  switch (eventNameLowerCase.toString()) {
    case "borg":
      embed.addFields(
...
function createOfficerEmbed(purpose) {
  const officerData = getEventData.findOfficerData(purpose);
